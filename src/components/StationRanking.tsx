...
export const StationRanking = ({ rank, type, lineColor, usage }: StationRankingProps) => {
  ...
  const updatePosition = () => {
    if (spanRef.current) {
      ...
      setPosition({
        top: rect.bottom + 8 - 120, // 8px gap
        left: type === 'usage' ? rect.left - 210 : rect.left - 150
      })
    }
  }
  ...
  return (<>
    ...
    <div 
      className={cn(
        "fixed z-50 w-[285px] p-2 rounded-lg border-1 bg-white/80 backdrop-blur-sm transition-all duration-200",
